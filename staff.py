...
class Staff(commands.Cog):
    ...
    @commands.command(description="DMs Member")
    @commands.has_role("Owner")
    async def dm(self, ctx: commands.Context, member: disnake.Member = None, *, content=None):
        await member.send(content)
        await ctx.send(f"I successfully sent message with content ``{content}`` to {member} (ID: {member.id})")
